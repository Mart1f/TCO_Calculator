...
class ResidualValueCalculator(System):
    ...
    def compute_external_factors(self):
        ...
        self.cO2_taxes_factor = self._countries_data[country]["external_factors"]["CO2_taxes_factor"][type_energy]
        self.subsidies_factor = self._countries_data[country]["external_factors"]["subsidies_factor"][type_energy]

        growth_rate = 0.04
        years = vp.current_year - vp.year_purchase
        P_ref = self._countries_data[country]["energy"]["energy_price_c_e"][type_energy]
        P_energy_t = P_ref * ((1 + growth_rate) ** years)
        delta_P_energy = (P_energy_t - P_ref) / P_ref

        Risk_CO2 = min(1.0, 0.3 + 0.02 * years)
        
        self.subsidies = self._countries_data[country]["subsidies"]["2025"]["medium"]["vehicle_subsidies"][type_energy]
        purchase_cost = vp.purchase_cost
        if purchase_cost > 0:
            incentive = self.subsidies / purchase_cost
        else:
            incentive = 0.0

        # Total external_factors
        self.total_external_factors = 1+self.energy_price_factor*delta_P_energy+ Risk_CO2*self.cO2_taxes_factor + incentive*self.subsidies_factor
